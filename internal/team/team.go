...
type Agent struct {
	...
	Provider     string   `yaml:"provider"`      // LLM provider: anthropic|google|openai|mistral
...
func (t *Team) Validate() error {
	...
	for i, agent := range t.Agents {
		...
		if !validProviders[agent.Provider] {
			return fmt.Errorf("agent '%s' has invalid provider '%s' — use: anthropic, google, openai, mistral", agent.Name, agent.Provider)
		}
		if agent.Model == "" {
